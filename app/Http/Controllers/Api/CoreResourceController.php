...
use App\Models\{Category, City, Customer, DeliveryPricingRule, DeliveryZone, Driver, Order, OrderItem, Product, ProductiveFamily, Store, Vehicle, Wallet, WalletTransaction};
...
class CoreResourceController extends Controller
{
    ...
    public function index(Request $request, string $resource): JsonResponse
    {
        ...
        foreach ($filterable as $field) if ($request->filled($field)) $query->where($field, $request->input($field));
        $sort = $request->string('sort', 'created_at')->toString();
        $direction = $request->string('direction', 'desc')->lower()->toString() === 'asc' ? 'asc' : 'desc';
        if (in_array($sort, array_merge(['id','created_at','updated_at'], array_keys($rules)), true)) $query->orderBy($sort, $direction);
    ...
    public function store(Request $request, string $resource): JsonResponse
    {
        ...
        return response()->json(['message'=>'تمت الإضافة بنجاح.','data'=>new CoreResource($record)], 201);
    }
    ...
    public function update(Request $request, string $resource, int $id): JsonResponse
    {
        ...
        $data = $request->validate($this->updateRules($rules, $record->getTable(), $id));
        DB::transaction(fn () => $record->update($data));
        return response()->json(['message'=>'تم التحديث بنجاح.','data'=>new CoreResource($record->fresh())]);
    }
    ...
    private function config(string $resource): array { abort_unless(isset(self::CONFIG[$resource]), 404, 'المورد غير معروف.'); return self::CONFIG[$resource]; }
    private function applySearch(Builder $query, Request $request, array $fields): void { if (!$request->filled('search')) return; $term=$request->string('search')->toString(); $query->where(fn(Builder $q)=>collect($fields)->each(fn($field)=>$q->orWhere($field,'like',"%{$term}%"))); }
    private function updateRules(array $rules, string $table, int $id): array { foreach($rules as $field=>$rule){ $parts=is_array($rule)?$rule:explode('|',$rule); $parts=array_map(function($part)use($table,$field,$id){ if(is_string($part)&&str_starts_with($part,'unique:')) return Rule::unique($table,$field)->ignore($id); return $part; },$parts); $rules[$field]=array_merge(['sometimes'],$parts); } return $rules; }
