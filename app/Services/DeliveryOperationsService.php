...
use App\Models\{DeliveryAssignment, Driver, Order, OrderStatusHistory};
...
class DeliveryOperationsService
{
    ...
    public function transition(Order $order, string $status, ?string $note, ?int $userId): Order
    {
        ...
        if(!in_array($status,$allowed,true)) throw ValidationException::withMessages(['status'=>'الانتقال بين حالتي الطلب غير مسموح.']);
        return DB::transaction(function()use($order,$status,$note,$userId){
            $from=$order->status; $updates=['status'=>$status];
            $column=match($status){'accepted'=>'accepted_at','preparing'=>'preparing_at','ready'=>'ready_at','picked_up'=>'picked_up_at','delivered'=>'delivered_at','cancelled'=>'cancelled_at',default=>null};
            if($column)$updates[$column]=now();
            $order->update($updates);
            OrderStatusHistory::create(['order_id'=>$order->id,'from_status'=>$from,'to_status'=>$status,'note'=>$note,'changed_by'=>$userId]);
            if(in_array($status,['delivered','cancelled'],true)&&$order->driver_id){ Driver::whereKey($order->driver_id)->where('active_orders_count','>',0)->decrement('active_orders_count'); }
            return $order->fresh();
        });
    }
    ...
    {
        if(!$force){
            if(!$driver->is_online||!in_array($driver->status,['active','approved'],true)) throw ValidationException::withMessages(['driver_id'=>'المندوب غير متاح حاليًا.']);
            if($order->city_id&&$driver->city_id&&$order->city_id!==$driver->city_id) throw ValidationException::withMessages(['driver_id'=>'المندوب خارج مدينة الطلب.']);
        }
        return DB::transaction(function()use($order,$driver,$userId){
            if($order->driver_id&&$order->driver_id!==$driver->id) Driver::whereKey($order->driver_id)->where('active_orders_count','>',0)->decrement('active_orders_count');
    ...
    public function autoAssign(Order $order, ?int $userId): DeliveryAssignment
    {
        ...
        if(!$driver) throw ValidationException::withMessages(['driver_id'=>'لا يوجد مندوب متاح حاليًا.']);
