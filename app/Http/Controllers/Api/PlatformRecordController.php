...
class PlatformRecordController extends Controller
{
    ...
    {
        abort_unless(in_array($resource, self::RESOURCES, true), 404, 'المورد غير معروف.');
    }
    ...
    public function show(string $resource, PlatformRecord $record): JsonResponse
    {
        $this->guard($resource);
        abort_unless($record->resource === $resource, 404);
    ...
    public function update(Request $request, string $resource, PlatformRecord $record): JsonResponse
    {
        ...
        $this->audit($request, $record, 'updated', $before, $record->fresh()->toArray());

        return response()->json(['message' => 'تم تحديث السجل بنجاح.', 'data' => $record->fresh()]);
    }

    public function destroy(Request $request, string $resource, PlatformRecord $record): JsonResponse
    {
        $this->guard($resource);
        abort_unless($record->resource === $resource, 404);
        $before = $record->toArray();
        $record->delete();
        $this->audit($request, $record, 'deleted', $before, null);
    ...
    {
        $this->guard($resource);
        $data = $request->validate(['ids' => ['required', 'array', 'min:1'], 'ids.*' => ['integer'], 'action' => ['required', 'in:delete,archive,activate,deactivate,approve,reject'], 'status' => ['nullable', 'string']]);
        ...
        foreach ($records as $record) {
            ...
            if ($data['action'] === 'delete') {
                ...
                $status = $data['status'] ?? match ($data['action']) {
                    'archive' => 'archived','activate' => 'active','deactivate' => 'inactive','approve' => 'approved','reject' => 'rejected'
                };
                $record->update(['status' => $status, 'updated_by' => $request->user()?->id]);
            } $this->audit($request, $record, 'bulk_'.$data['action'], $before, $record->fresh()?->toArray());
        }

        return response()->json(['message' => 'تم تنفيذ الإجراء الجماعي.', 'affected' => $records->count()]);
    }

    private function audit(Request $request, PlatformRecord $record, string $action, ?array $before, ?array $after): void
    {
        PlatformAuditLog::create(['user_id' => $request->user()?->id, 'resource' => $record->resource, 'record_id' => $record->id, 'action' => $action, 'before' => $before, 'after' => $after, 'ip_address' => $request->ip(), 'user_agent' => Arr::limit($request->userAgent() ?? '',1000)]);
    }
}
