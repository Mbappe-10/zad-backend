...
class WorkforceController extends Controller
{
    public function dashboard(): JsonResponse
    {
        ...
        $completed=(int)($taskCounts['completed']??0); $failed=(int)($taskCounts['failed']??0); $decided=$completed+$failed+(int)($taskCounts['rejected']??0);
    ...
    public function employees(Request $request): JsonResponse
    {
        ...
        if($request->filled('search')){$s=$request->string('search');$q->where(fn($x)=>$x->where('name_ar','like',"%$s%")->orWhere('employee_number','like',"%$s%")->orWhere('email','like',"%$s%"));}
    ...
    {
        $data=$this->validateEmployee($request); $data['employee_number']=$data['employee_number']??'EMP-'.Str::upper(Str::random(8));
    ...
    {
        $employee->delete(); return response()->json(['message'=>'تم حذف ملف الموظف.']);
    }
    ...
    public function tasks(Request $request): JsonResponse
    {
        ...
        if($request->filled('status'))$q->where('status',$request->string('status'));
        if($request->filled('employee_id'))$q->where('digital_employee_id',$request->integer('employee_id'));
        return response()->json($q->paginate(min(100,max(10,$request->integer('per_page',20)))));
    }
    ...
    public function cancelTask(Request $request,DigitalEmployeeTask $task): JsonResponse
    {
        ...
        $from=$task->status; $task->update(['status'=>'cancelled','cancelled_at'=>now()]);
    ...
    public function retryTask(Request $request,DigitalEmployeeTask $task): JsonResponse
    {
        ...
        $from=$task->status; $task->update(['status'=>'queued','error_message'=>null,'cancelled_at'=>null]);
    ...
    {
        $q=AutomationRule::with('employee:id,name_ar')->latest(); if($request->filled('active'))$q->where('is_active',$request->boolean('active'));
    ...
    public function runRule(Request $request,AutomationRule $rule): JsonResponse
    {
        ...
        try {
            $actions=$rule->actions??[]; $created=[];
            if($rule->digital_employee_id){foreach($actions as $action){$created[]=DigitalEmployeeTask::create(['digital_employee_id'=>$rule->digital_employee_id,'title'=>$action['title']??$rule->name,'instructions'=>$action['instructions']??'تنفيذ قاعدة التشغيل: '.$rule->name,'priority'=>$action['priority']??'medium','input'=>$request->input('input',[])])->id;}}
            $run->update(['status'=>'completed','output'=>['created_task_ids'=>$created,'actions_count'=>count($actions)],'completed_at'=>now()]); $rule->update(['last_triggered_at'=>now()]);
        } catch (\Throwable $e) { $run->update(['status'=>'failed','error_message'=>$e->getMessage(),'completed_at'=>now()]); throw $e; }
    ...
    public function events(Request $request): JsonResponse
    {
        ...
        if($request->filled('event_type'))$q->where('event_type',$request->string('event_type'));
