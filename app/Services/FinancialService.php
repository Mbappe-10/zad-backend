...
use App\Models\{FinancialLedgerEntry, Payment, Payout, Refund, Wallet, WalletTransaction};
...
class FinancialService
{
    public function credit(Wallet $wallet, float $amount, string $type, string $description, ?object $source, ?int $userId): WalletTransaction
    {
        return DB::transaction(function () use ($wallet,$amount,$type,$description,$source,$userId) {
            ...
            if ($wallet->is_frozen) throw ValidationException::withMessages(['wallet'=>'المحفظة مجمدة.']);
            $wallet->available_balance = round((float)$wallet->available_balance + $amount, 2); $wallet->save();
    ...
    public function requestPayout(Wallet $wallet, array $data, ?int $userId): Payout
    {
        return DB::transaction(function () use ($wallet,$data,$userId) {
            $wallet=Wallet::query()->lockForUpdate()->findOrFail($wallet->id);
            if ($wallet->is_frozen) throw ValidationException::withMessages(['wallet'=>'المحفظة مجمدة.']);
            $amount=(float)$data['amount']; if ($amount <= 0 || $amount > (float)$wallet->available_balance) throw ValidationException::withMessages(['amount'=>'الرصيد المتاح غير كافٍ.']);
            $wallet->available_balance=round((float)$wallet->available_balance-$amount,2); $wallet->pending_balance=round((float)$wallet->pending_balance+$amount,2); $wallet->save();
    ...
    public function decidePayout(Payout $payout, string $decision, ?int $userId): Payout
    {
        return DB::transaction(function () use ($payout,$decision,$userId) {
            $payout=Payout::query()->lockForUpdate()->findOrFail($payout->id); if ($payout->status!=='pending') throw ValidationException::withMessages(['status'=>'تم اتخاذ قرار على طلب الصرف مسبقًا.']);
            $wallet=Wallet::query()->lockForUpdate()->findOrFail($payout->wallet_id);
            if ($decision==='reject') { $wallet->pending_balance-=$payout->amount; $wallet->available_balance+=$payout->amount; $wallet->save(); $payout->update(['status'=>'rejected','approved_by'=>$userId,'approved_at'=>now()]); }
            else { $wallet->pending_balance-=$payout->amount; $wallet->save(); $payout->update(['status'=>'paid','approved_by'=>$userId,'approved_at'=>now(),'paid_at'=>now()]); $this->ledger('cash','debit',(float)$payout->net_amount,'صرف مستحقات',$payout,$userId); }
            return $payout->fresh();
        });
    }
    ...
    public function refund(Payment $payment, array $data, ?int $userId): Refund
    {
        return DB::transaction(function () use ($payment,$data,$userId) {
            ...
            $amount=(float)$data['amount']; if ($amount<=0 || $refunded+$amount>(float)$payment->gross_amount) throw ValidationException::withMessages(['amount'=>'قيمة الاسترداد تتجاوز المبلغ القابل للاسترداد.']);
            ...
            $this->ledger('customer_refunds','debit',$amount,'استرداد دفعة',$refund,$userId); return $refund;
        });
    }
